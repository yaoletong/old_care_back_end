...
import app.mod_camera.comtrollers as con

from app.mod_camera.oldcare.track import CentroidTracker
from app.mod_camera.oldcare.track import TrackableObject
...
from app.models import InvadeInfo



class Camera_invade(BaseCamera):
    video_source = 'F:/study/small_semester3/暑期实训2022/代码/任务19.禁止区域入侵检测2/任务19.禁止区域入侵检测/tests/yard_01.mp4'

    def __init__(self):
        if os.environ.get('OPENCV_CAMERA_SOURCE'):
            Camera_invade.set_video_source(int(os.environ['OPENCV_CAMERA_SOURCE']))
        super(Camera_invade, self).__init__()

    @staticmethod
    def set_video_source(source):
        Camera_invade.video_source = source
    ...
    def frames():
        ...
        # 全局变量
        prototxt_file_path = './app/mod_camera/models/mobilenet_ssd/MobileNetSSD_deploy.prototxt'
        ...
        model_file_path = './app/mod_camera/models/mobilenet_ssd/MobileNetSSD_deploy.caffemodel'
        output_fence_path = './app/static/smile'
        ...
        camera = cv2.VideoCapture(Camera_invade.video_source)
                    ...
                    if not to.counted:
                        ...
                        # center line, count the object
                            ...
                            if countTime % 10 == 0:
                                # 截图
                                cv2.imwrite(
                                    os.path.join(output_fence_path,
                                                 'snapshot_%s.jpg' % (time.strftime('%Y%m%d_%H%M%S'))),
                                    frame)  # snapshot
                                stranger = InvadeInfo()
                                stranger.date = current_time
                                # stranger.oldperson_id = int(name)
                                stranger.invade_info = event_desc
                                stranger.address = event_location
                                stranger.record = 'http://192.168.0.101:8085/static/smile/' + 'snapshot_%s.jpg' % (time.strftime('%Y%m%d_%H%M%S'))
                                # db.session.add(stranger)
                                # # data.imgset_dir = data.imgset_dir+'\\'+str(data.id)
                                # db.session.commit()
                                con.update_insert_data(stranger)
                                con.send_emailInvade()
